...
const edges = [
  ["entrance_north", "concourse_west"], ["entrance_north", "concourse_east"],
  ["entrance_south", "concourse_west"], ["entrance_south", "concourse_east"],
  ["concourse_west", "food_court_1"], ["concourse_west", "restroom_1"],
  ["concourse_west", "seating_section_a"], ["concourse_west", "seating_section_b"],
  ["concourse_west", "emergency_exit_west"], ["concourse_west", "trophy_room"],
  ["concourse_east", "food_court_2"], ["concourse_east", "restroom_2"],
  ["concourse_east", "seating_section_c"], ["concourse_east", "seating_section_d"],
  ["concourse_east", "emergency_exit_east"], ["concourse_east", "fan_zone"],
  ["trophy_room", "seating_section_a"], ["fan_zone", "seating_section_c"],
  ["concourse_west", "merch_store_1"], ["concourse_east", "merch_store_2"]
];
...
export const VenueMap: React.FC<VenueMapProps> = ({
  ...
  for(let i = 0; i < recommendedPath.length - 1; i++){
    pathEdges.push(`${recommendedPath[i]}-${recommendedPath[i+1]}`);
    pathEdges.push(`${recommendedPath[i+1]}-${recommendedPath[i]}`);
  }

  const nodeInsights: Record<string, any> = {};
  heatmap.forEach(h => nodeInsights[h.id] = h);
  ...
  return (
    <div className="map-container animate-enter" style={{ marginBottom: "2rem" }}>
      ...
      {weather === "rain" && (
        <div style={{ position: "absolute", top: 24, right: 24, background: "rgba(0,0,0,0.8)", color: "white", padding: "6px 16px", borderRadius: "20px", fontSize: "0.85rem", fontWeight: 600 }}>Active Weather Monitored</div>
      )}

      {/* Accessibility: Improved `<svg>` tags with ARIA role and descriptions */}
      <svg 
        viewBox="0 0 800 600" 
      ...
      >
        ...
        {edges.map((edge, i) => {
          const p1 = nodeCoords[edge[0]];
          const p2 = nodeCoords[edge[1]];
          const isHighlighted = pathEdges.includes(`${edge[0]}-${edge[1]}`);
          
          let color = "#e2e8f0";
          if (isHighlighted) color = emergency ? "var(--accent-red)" : "var(--accent-blue)";
        ...
        {Object.entries(nodeCoords).map(([key, point]) => {
          ...
          return (
            <g key={key} role="button" aria-pressed={inPath} tabIndex={view === "admin" ? 0 : -1} 
               onClick={() => view === "admin" ? triggerCongestion(key) : undefined}
              ...
              {(isStart || isEnd) && view === "attendee" && (
                 <circle cx={point.x} cy={point.y} r="8" fill={isStart ? "var(--accent-blue)" : "var(--accent-purple)"} className="radar-ring" style={{ transformOrigin: `${point.x}px ${point.y}px` }} />
              )}
              ...
              <circle 
                ...
                stroke={inPath && view === "attendee" ? (emergency ? "var(--accent-red)" : "var(--accent-blue)") : "var(--border-light)"}
                strokeWidth={inPath ? "3" : "2"}
                style={{ transition: "all 0.2s", cursor: view === "admin" ? "pointer" : "default" }}
              >
                {view === "admin" && <title>Admins: Click to trigger load spike.</title>}
              </circle>
              
              {(isStart || isEnd) && (
                 <g className="bounce-text">
                    <text x={point.x} y={point.y - 14} fill="var(--text-primary)" fontSize="12" fontWeight="800" textAnchor="middle">{isStart ? "START" : "DESTINATION"}</text>
                 </g>
              )}
              <text x={point.x} y={point.y + 20} fill="var(--text-primary)" fontSize="12" textAnchor="middle" fontWeight="600">{point.label}</text>
            </g>
          )
        })}
      </svg>
